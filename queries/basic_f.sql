...
SELECT id oldest_id, firstname oldest_firstname, lastname oldest_lastname, birthdate oldest_birthdate, year
FROM (
    SELECT id, lastname, firstname, year, birthdate, RANK() OVER (PARTITION BY year ORDER BY birthdate ASC) r
    FROM (
        SELECT pl.id, p.lastname, p.firstname, psea.year, pl.birthdate
        FROM players pl
        JOIN player_seasons psea ON psea.player_id = pl.id
...
WHERE ROWID IN (SELECT MAX(ROWID) FROM youngest_players GROUP BY year);
